 ...
 * Config : RPC_URL di .env atau diisi manual di bawah
 *
...
// ─── CONFIG ───────────────────────────────────────────────────────────────────
// Ganti RPC sesuai chain target (atau taruh di .env sebagai RPC_URL)
const RPC_URL = process.env.RPC_URL || "https://mainnet.infura.io/v3/YOUR_KEY";

// Hash query GraphQL OpenSea — bisa berubah kalau OpenSea update JS bundle mereka
const MINT_MODULE_HASH   = "98b96c9357f51630dc14c3bcab0de47684337a0aa726277b82820d6ee354217d"; // MintModuleQuery
const ELIG_HASH          = "e1b54354df0d26d39c6b81429bd5e5d37749eaa4bdc027f987128f8c1e7d2308"; // DropEligibilityQuery
const MINT_TIMELINE_HASH = "4e1ef4c8393b2025b5e9a72621b61af83b1bb2d6cf1c5614ef0f4be7d0c2cde8"; // MintActionTimelineQuery
...
// ─── AUTH (sama persis dari cekelig.js) ───────────────────────────────────────
...
async function fetchMintCalldata(address, cookieJar, stageIndex, quantity) {
  // MintActionTimelineQuery — operasi yang return transactionSubmissionData
  // berisi: toAddress (contract), calldata, value (harga ETH)
  ...
  // Coba ekstrak transactionSubmissionData dari berbagai path response
...
async function submitMint(privateKey, txData) {
  ...
  const tx = {
    ...
    gasLimit        : 300_000n, // safe default buat SeaDrop mint
  };

  // Estimasi gas buat konfirmasi sebelum kirim
  try {
    const estimated = await provider.estimateGas({ ...tx, from: wallet.address });
    tx.gasLimit = (estimated * 120n) / 100n; // +20% buffer
...
async function main() {
  ...
  if (!RPC_URL || RPC_URL.includes("YOUR_KEY")) {
    console.log(`${RED}[!] Set RPC_URL dulu — di .env atau langsung di skrip ini${RESET}`);
    process.exit(1);
  }
  ...
  console.log(`\n[*] Mulai proses ${selected.length} wallet...\n`);
  ...
  for (let i = 0; i < selected.length; i++) {
    ...
    try {
      ...
      // Filter stage sesuai pilihan user
      ...
      for (const stage of targetStages) {
        ...
        try {
          ...
          console.log(`    [stage ${stage.stageIndex}] submit tx...`);
  ...
  const success = results.filter(r => r.status === "done" &&
    r.stages?.some(s => s.status === "success")).length;
  console.log(`\n=== SELESAI ===`);
